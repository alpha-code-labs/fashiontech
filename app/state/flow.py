...
class FlowEngine:
            ...
            if c == "top" and kk == "length":
                length_prompts = {
                    "crop": (
                        ...
                        "1. Shorten the top into a CROP TOP.\n"
                        "2. The new hem MUST end at the mid-ribcage (lowest rib level), well above the navel.\n"
                        "3. Remove ALL fabric below this new hemline.\n"
                        "RESULT:\n"
                        "1. The midriff area between the shortened top and the pants waistband is visible.\n"
                        "2. The gap between top hem and pants waistband must look natural.\n"
                        "BOUNDARY LOGIC (CRITICAL):\n"
                        "1. Preserve the lower garment (pants/skirt) exactly as it appears.\n"
                        "2. DO NOT stretch the lower garment upward.\n"
                        "3. Keep the same color, fabric, pattern, neckline, and sleeves."
                    ),
                    "regular": (
                        "PRIORITY: Apply this modification fully and visibly.\n"
                        "RECONSTRUCTION (LENGTH):\n"
    ...
    async def _send_next_buy_option(self, wa_id: str) -> None:
        ...
        if c == "dress":
            ...
            # Length + fit + waist_rise already in design modification menu
            await self._send_buy_confirm(wa_id)
        elif c == "jumpsuit":
            await self._send_waist_def_selection(wa_id)
        elif c == "shirts":
